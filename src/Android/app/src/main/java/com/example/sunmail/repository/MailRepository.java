...
public class MailRepository {
    ...
    /**
     * Search mails with a query
     */
    public void searchMails(MutableLiveData<List<Mail>> mailsLiveData, String query) {
        Call<List<Mail>> call = mailApi.searchMails(query);
        call.enqueue(new Callback<List<Mail>>() {
            @Override
            public void onResponse(Call<List<Mail>> call, Response<List<Mail>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    Log.e("MailRepository", "Search error: " + response.code());
                    mailsLiveData.postValue(Collections.emptyList());
                }
            }

            @Override
            public void onFailure(Call<List<Mail>> call, Throwable t) {
                Log.e("MailRepository", "Search network error", t);
                mailsLiveData.postValue(Collections.emptyList());
            }
        });
    }
